...
function show_comments($comments, $parent_id = -1) {
    ...
    foreach ($comments as $comment) {
        if ($comment['parent_id'] == $parent_id) {
            ...
            <div class="comment">
                <div>
                    <h3 class="name"><img src="img/user.png" class="comment-img" alt="MaxBeatx"> ' . htmlspecialchars($comment['name'], ENT_QUOTES) . '</h3>
                     <span class="date"><i class="far fa-clock"></i> ' . time_elapsed_string($comment['submit_date']) . '</span>
                </div>
...
if (isset($_GET['page_id'])) {
    ...
    if (isset($_POST['name'], $_POST['content'])) {
        ...
        exit('<span style=" color: green; font-size: 12px;">Your comment has been submitted!</span>');
    }
...
<div style="float: right; position: absolute; top: 210px; right: 12px;" class="fa-comment"><a href="#comment_container"><i class="fa fa-comment" style="color: #b1060f;"></i> <span><?=$comments_info['total_comments']?></span></a></div>
